        ...
        <div className="max-w-lg w-full bg-white rounded-3xl shadow-2xl p-10 text-center relative z-10">
          ...
          <div className="mb-8 px-4 py-2 rounded-full inline-flex items-center gap-2 border border-slate-100 bg-slate-50 text-[10px] font-black uppercase tracking-widest">
            {submitStatus === 'submitting' && (
               <><div className="w-2 h-2 bg-amber-400 rounded-full animate-ping"></div><span className="text-amber-600">Saving results to Sheets...</span></>
            )}
            {submitStatus === 'success' && (
               <><div className="w-2 h-2 bg-emerald-500 rounded-full"></div><span className="text-emerald-600">Results Synced Successfully!</span></>
            )}
            {submitStatus === 'error' && (
               <><div className="w-2 h-2 bg-rose-500 rounded-full"></div><span className="text-rose-600">Sync Failed (Check App Script Deployment)</span></>
            )}
            {submitStatus === 'idle' && !GOOGLE_SHEET_URL && (
               <span className="text-slate-400 italic">Sheet sync not configured</span>
            )}
          </div>
